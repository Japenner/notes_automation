...
module NotesAutomation
  module FileProcessingUtilities
    ...
    def build_file_tree_helper(node, ignore, root_path, index, attachments, use_git_times)
      ...
      Dir.entries(full_dir).sort_by { |name| name.downcase }.each do |entry|
        next if entry == '.' || entry == '..'
        if ignore.include?(entry)
          info("Ignoring file", entry)
          next
        end
        if entry == "Untitled.md" || entry == "Untitled"
        ...
        if File.directory?(entry_path)
          ...
          path = rel_entry
          subtree = build_file_tree_helper(FileTree.new(dir: path), ignore, root_path, index, attachments, use_git_times)
          ...
          if page_or_attach.is_a?(Page)
            ...
            if page_or_attach.frontmatter["draft"]
              next
            end
